defmodule Flop.Phoenix do
  ...
  # every query parameter to_query/2 can produce
  @flop_param_keys ~w(
    after before filters first last limit offset order_by order_directions
    page page_size
  )
  ...
  The `op` option only sets the value of the hidden `op` input that the form
  renders, but it does not prevent a user from submitting a different value.
  Flop validates the value and reject operators that are not allowed for a
  field. The validation error includes a list of allowed operators. See
  `Flop.Filter.allowed_operators/2`.

  ## Handling filter form parameters

  The form renders an input for every configured field, and the browser submits
  a parameter for every input, whether the user filled it in or not. LiveView
  adds a `_target` parameter naming the input that triggered the event, and a
  `_unused_*` parameter for every input the user has not interacted with yet.

  If you handle the form with `c:Phoenix.LiveView.handle_event/3` and patch the
  URL, you probably want to remove these parameters.

  To remove the `_target` parameter:
  ...
  Alternatively, build the form with an `:as` option. The form parameters are
  then nested under that name, while `_target` remains a sibling of it, so
  matching on the name leaves the parameter behind.

      form = to_form(@meta, as: :filter)

      def handle_event("update-filter", %{"filter" => params}, socket) do
        {:noreply, push_patch(socket, to: ~p"/pets?\#{params}")}
      end

  The query string is the same either way, since the form name is not part of
  the nested parameters.

  To remove the `_unused_*` parameters, you can set the `phx-no-unused-field`
  attribute on the form. This attribute is available since LiveView 1.2.
  Note that the input component generated by Phoenix uses these parameters to
  hide the errors of a field until the user has interacted with it. Without
  them, the errors are rendered right away. See
  [error feedback](https://phoenix-live-view.hexdocs.pm/form-bindings.html#error-feedback)
  for details.

  In older LiveView versions, you need to remove the `_unused_*` parameters
  manually. This example removes both the `_target` and the `_unused_*`
  parameters:
      ...
      defp clean_params(params) when is_map(params) do
        params
        |> Map.reject(fn {key, _} ->
          key == "_target" or String.starts_with?(key, "_unused_")
        end)
        |> Map.new(fn {key, value} -> {key, clean_params(value)} end)
      end

      defp clean_params(params) when is_list(params) do
        Enum.map(params, &clean_params/1)
      end

      defp clean_params(value), do: value
  """
  @doc since: "0.12.0"
  @doc section: :components
  ...
  A Flop parameter that exists only in the path is removed, so that the
  parameters always describe the given Flop struct.

      iex> flop = %Flop{first: 10, after: "eyJpZCI6IDF9"}
      iex> path = build_path("/pets?page=3&species=dogs", flop)
      iex> %URI{path: parsed_path, query: parsed_query} = URI.parse(path)
      iex> {parsed_path, URI.decode_query(parsed_query)}
      {"/pets", %{"after" => "eyJpZCI6IDF9", "first" => "10", "species" => "dogs"}}
  ...
  def build_path(uri, flop_params, _opts)
    ...
    # The decoded query has string keys, the Flop parameters atom keys.
    # Every parameter the Flop struct owns is dropped before merging.
    flop_params =
